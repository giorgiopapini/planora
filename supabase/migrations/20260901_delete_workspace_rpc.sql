...
begin
  ...
  if not public.is_workspace_admin(p_workspace_id) then raise exception 'Only a workspace administrator can delete this workspace'; end if;
  ...
  delete from public.sprint_tasks where task_id in (select t.id from public.tasks t join public.projects p on p.id=t.project_id where p.workspace_id=p_workspace_id);
