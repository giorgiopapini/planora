...
import { Avatar, Badge, Button, Card, DateInput, DeleteIconButton, EditIconButton, Modal, MultiSelect, Select, Textarea } from "@/components/ui";
...
export function Kanban() {
  ...
  return <>
    ...
    <Modal open={isAdding} title="Add a task" description="Add the details your team needs to get started." onClose={() => setIsAdding(false)}><form onSubmit={addTask} className="space-y-5">
      ...
      <DateInput id="new-task-date" label="Due date" value={form.dueDate} onChange={(event) => setForm({ ...form, dueDate: event.target.value })} />
      <Select id="new-task-priority" label="Priority" value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value as Priority })}>{["Low", "Medium", "High", "Urgent"].map((priority) => <option key={priority}>{priority}</option>)}</Select>
      ...
      <Textarea id="new-task-detail" label="Description" value={form.detail} onChange={(event) => setForm({ ...form, detail: event.target.value })} placeholder="Add more context (optional)" />
    ...
    <Modal open={Boolean(selectedTask)} title={selectedTask?.task.title || "Task details"} description="Click the pen icon beside any field to edit it." onClose={() => { setSelectedTask(null); setEditing(null); }} headerAction={<DeleteIconButton type="button" onClick={() => { if (selectedTask) { setSelectedTask(null); setEditing(null); setTaskToDelete(selectedTask); } }} aria-label="Delete task" />}>{selectedTask && <TaskDetails task={selectedTask.task} column={selectedTask.column} editing={editing} setEditing={setEditing} updateTask={updateTask} closeDetails={() => { setSelectedTask(null); setEditing(null); }} />}</Modal>
...
function TaskDetails({ task, column, editing, setEditing, updateTask, closeDetails }: { task: Task; column: ColumnId; editing: EditableField | null; setEditing: (field: EditableField | null) => void; updateTask: (id: number, changes: Partial<Task>, from?: ColumnId, to?: ColumnId) => void; closeDetails: () => void }) {
  const editButton = (field: EditableField, label: string) => <EditIconButton type="button" onClick={() => setEditing(editing === field ? null : field)} aria-label={`Edit ${label}`} />;
...
function formatDate(date: string) { if (!date) return "No due date"; const match = date.match(/^(\d{4})-(\d{2})-(\d{2})$/); if (!match) return "No due date"; const [, year, month, day] = match; return new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeZone: "UTC" }).format(new Date(`${year}-${month}-${day}T00:00:00Z`)); }
