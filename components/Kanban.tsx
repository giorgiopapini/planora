      ...
      <div className="grid gap-px bg-border md:grid-cols-2 xl:grid-cols-4">
        {columns.map((status) => {
          ...
          return (
            <section
              key={status}
              className="min-w-0 bg-subtle p-4"
            ...
            >
              <div className="mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant={
                      status.toLowerCase() === "completed" ? "success" : "count"
                    }
                  >
                    {columnTasks.length}
                  </Badge>
...
function TaskCard({
  ...
  const content = (
    <>
      <div className="flex items-start justify-between gap-3">
        <h5 className="text-sm font-medium leading-5 text-primary">
          {task.title}
        </h5>
        <span
  ...
  return (
    <article
      ...
      onClick={onClick}
      className={`group rounded-lg border border-border bg-surface p-4 transition-colors duration-120 hover:border-border-strong ${onClick ? "cursor-pointer" : ""}`}
...
function columnDescription(status: string) {
  const normalized = status.toLowerCase();
  if (normalized === "completed" || normalized === "done")
    return "Shipped and signed off";
  if (normalized.includes("review")) return "Awaiting feedback";
