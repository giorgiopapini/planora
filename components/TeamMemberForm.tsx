...
import { Avatar, Badge, Button, IconButton, Card, Modal } from "@/components/ui";
...
export function TeamMemberForm({ workspaceId, members: initialMembers, roles: initialRoles }: { workspaceId: string; members: Member[]; roles: Role[] }) {
  ...
  return <>
    ...
    <Modal open={rolesOpen} title="Handle roles" description="Create, rename, or remove workspace roles." onClose={() => { setRolesOpen(false); setEditingRole(null); }}><div className="space-y-5"><div><label htmlFor="new-role" className="sr-only">New role</label><div className="flex items-center gap-3"><input id="new-role" value={newRole} onChange={(event) => { setNewRole(event.target.value); setError(""); }} placeholder="Add a new role" className={`${inputClass} flex-1`} /><Button type="button" onClick={addRole}>Add</Button></div></div><div className="divide-y divide-border rounded-lg border border-border">{roles.map((role) => <div key={role.id} className="flex min-h-14 items-center gap-3 px-3"><div className="min-w-0 flex-1">{editingRole?.id === role.id ? <input autoFocus value={roleDraft} onChange={(event) => setRoleDraft(event.target.value)} className={inputClass} aria-label={`Rename ${role.name}`} /> : <Badge variant="count">{role.name}</Badge>}</div><div className="flex items-center gap-1">{!role.isSystem && (editingRole?.id === role.id ? <button type="button" onClick={saveRole} className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-md text-secondary hover:bg-muted hover:text-primary" aria-label={`Save ${role.name}`}>✓</button> : <button type="button" onClick={() => { setEditingRole(role); setRoleDraft(role.name); }} className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-md text-secondary hover:bg-muted hover:text-primary" aria-label={`Edit ${role.name}`}>✎</button>)}{!role.isSystem && <Button type="button" variant="danger" className="h-8 w-8 px-0" onClick={() => deleteRole(role)} aria-label={`Delete ${role.name}`}>🗑</Button>}</div></div>)}</div></div></Modal>

    <Modal open={Boolean(editingMember)} title={`Edit ${editingMember?.name || "team member"}`} description="Update this team member's workspace role." onClose={() => setEditingMember(null)}>{editingMember && <div className="space-y-5"><div className="space-y-1.5"><label htmlFor="edit-member-role" className="block text-sm font-medium">Workspace role</label><select id="edit-member-role" autoFocus value={editingMember.roleId} onChange={(event) => saveMemberRole(event.target.value)} className={inputClass}>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select></div><div className="flex justify-end border-t border-border pt-5"><Button type="button" variant="secondary" onClick={() => setEditingMember(null)}>Cancel</Button></div></div>}</Modal>
