...
export function TeamMemberForm({
  ...
  const [permissionKey, setPermissionKey] =
    useState<PermissionKey>("normal_user");
  ...
  const assignableRoles = roles.filter(
    (role) =>
      role.roleKey !== "owner" &&
      (permission === "owner_like" || role.permissionKey === "normal_user"),
  );
  const canManageMember = (member: Member) =>
    canManageMembers &&
    !member.isOwner &&
    (permission === "owner_like" || member.permissionKey === "normal_user");
  const canManageInvitation = (invitation: PendingInvitation) =>
    canManageMembers &&
    (permission === "owner_like" || invitation.permissionKey === "normal_user");
  const [roleId, setRoleId] = useState(
    assignableRoles.find((role) => role.name === "Member")?.id ||
      assignableRoles.find((role) => !role.isSystem)?.id ||
      assignableRoles[0]?.id ||
      "",
  );

  function showRoleError(actionError: unknown) {
    setRoleError(
        ...
        unknownRole
          ? current.map((member) =>
              member.roleId === role.id
                ? { ...member, roleId: unknownRole.id, role: unknownRole.name }
                : member,
            )
          : current,
      );
  ...
  function permissionLabel(value: PermissionKey) {
    return value === "owner_like"
      ? "Owner-like"
  ...
  async function saveMemberRole(nextRoleId: string) {
    ...
    try {
        ...
        current.map((member) =>
          member.id === editingMember.id
            ? {
                ...member,
                roleId: nextRoleId,
                role: nextRole?.name || member.role,
                permissionKey: nextRole?.permissionKey || member.permissionKey,
              }
            : member,
        ),
      );
      ...
      showMemberRoleError(actionError);
    }
  }
  ...
  async function deleteMember() {
    ...
    try {
      ...
      setMembers((current) =>
        current.filter((member) => member.id !== memberToDelete.id),
      );
      setMemberToDelete(null);
      setRemoveError("");
    } catch (actionError) {
      showRemoveError(actionError);
    }
  }
  ...
  async function deleteInvitation() {
    ...
    try {
      await dismissWorkspaceInvitation({
        workspaceId,
        invitationId: invitationToDelete.id,
      });
      setPendingInvitations((current) =>
        current.filter((invitation) => invitation.id !== invitationToDelete.id),
      );
  ...
  return (
    <>
      <Card>
        ...
        <div className="grid gap-1 p-6 sm:grid-cols-2">
          {members.map((member) => (
            <div
              key={member.id}
              className="flex min-w-0 items-center gap-3 rounded-lg p-3 transition-colors duration-120 hover:bg-subtle"
            >
          ...
          {pendingInvitations.filter(canManageInvitation).map((invitation) => (
            <div
              key={invitation.id}
              className="flex min-w-0 items-center gap-3 rounded-lg border border-warning-soft bg-warning-soft/40 p-3"
            >
              ...
              <IconButton
                ...
                aria-label={`Dismiss invitation for ${invitation.email}`}
              />
            </div>
          ))}
        </div>
                ...
                >
                  <div className="min-w-0 flex-1">
                    {editingRole?.id === role.id ? (
                      ...
                      <div className="flex min-w-0 items-center gap-2">
                        <Badge variant="count">{role.name}</Badge>
                        <span className="truncate text-xs text-tertiary">
                          {permissionLabel(role.permissionKey)}
                        </span>
                      </div>
                    )}
                  </div>
                    ...
                    {!role.isSystem &&
                      ...
                      (editingRole?.id === role.id ? (
                        <button
                          type="button"
                          onClick={saveRole}
                          className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-md text-secondary hover:bg-muted hover:text-primary"
