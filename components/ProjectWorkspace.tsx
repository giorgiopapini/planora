...
import { Avatar, AvatarGroup, Badge, Button, Calendar, Card, DateInput, GanttChart, IconButton, CardContent, CardDescription, CardHeader, CardTitle, Input, Modal, MultiSelect, ProjectHealth, Select, Textarea } from "@/components/ui";
...
type NewTaskForm = { title: string; detail: string; startDate: string; dueDate: string; priority: TaskPriority; assigneeId: string };

const priorities: TaskPriority[] = ["Low", "Medium", "High", "Urgent"];
const projectStatuses: ProjectStatus[] = ["Planning", "In progress", "On track", "Archived"];
const blankTask: NewTaskForm = { title: "", detail: "", startDate: new Date().toISOString().slice(0, 10), dueDate: "", priority: "Medium", assigneeId: "" };
...
export function ProjectWorkspace({ project: initialProject, selectedWorkspace, workspaceMembers = initialProject.team.map((member) => ({ userId: member.userId, name: member.name })) }: ProjectWorkspaceProps) {
  ...
  const [taskView, setTaskView] = useState<"list" | "gantt" | "calendar" | "kanban">("list");
  ...
  const ganttTasks = useMemo(() => project.taskList.filter((task) => task.dueDateIso).map((task) => ({ id: task.id, name: task.title, start: task.startDateIso, end: task.dueDateIso as string, progress: task.status.toLowerCase() === "completed" ? 100 : task.status.toLowerCase() === "in progress" ? 50 : 0, status: task.status, color: task.status.toLowerCase() === "completed" ? "dark" as const : task.status.toLowerCase() === "todo" ? "light" as const : "accent" as const })), [project.taskList]);
  const today = new Date().toISOString().slice(0, 10);
  const ganttStartDate = project.startDateIso > today ? project.startDateIso : today;
  const ganttEndDate = project.dueDateIso < today ? today : project.dueDateIso;
  const calendarEvents = useMemo(() => project.taskList.filter((task) => task.dueDateIso).map((task) => ({ id: task.id, date: task.startDateIso, endDate: task.dueDateIso as string, title: task.title, color: task.status.toLowerCase() === "completed" ? "dark" as const : task.status.toLowerCase() === "todo" ? "muted" as const : "accent" as const })), [project.taskList]);
  ...
  async function changeTask(taskId: string, changes: Partial<ProjectTask>) {
    ...
    if (!task) return;
    try {
      await updateTaskAction({ taskId, title: changes.title, description: changes.detail, startDate: changes.startDateIso, dueDate: changes.dueDate === undefined ? undefined : changes.dueDate === "No due date" ? null : toInputDate(changes.dueDate), priority: changes.priority, statusName: changes.status, assigneeIds: changes.assigneeIds });
      setProject((current) => {
  ...
  function openTaskEditor(task: ProjectTask) {
    setEditingTask(task);
    setTaskForm({ title: task.title, detail: task.detail === "No description provided." ? "" : task.detail, startDate: task.startDateIso, dueDate: task.dueDateIso || "", priority: task.priority, assigneeId: task.assigneeIds[0] || "" });
  }

  async function saveTask(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!editingTask || !taskForm.title.trim()) return;
    await changeTask(editingTask.id, { title: taskForm.title, detail: taskForm.detail, startDate: taskForm.startDate, dueDate: taskForm.dueDate || noDueDate, priority: taskForm.priority, assigneeIds: taskForm.assigneeId ? [taskForm.assigneeId] : [] });
    setEditingTask(null);
  }
  ...
  async function addTask(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const taskId = await createTask({ projectId: project.id, title: taskForm.title, description: taskForm.detail, startDate: taskForm.startDate, dueDate: taskForm.dueDate || null, priority: taskForm.priority, assigneeId: taskForm.assigneeId || undefined });
  ...
  return <div className="space-y-8">
    ...
    <Card><CardHeader className="flex-wrap"><div><CardTitle>Project tasks</CardTitle><CardDescription>Track the work, ownership, and delivery status in one place.</CardDescription></div><Badge variant="count">{project.taskList.length} tasks</Badge></CardHeader><CardContent><div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-border pb-5"><div className="inline-flex rounded-lg border border-border bg-subtle p-1" role="tablist" aria-label="Task visualization"><ViewTab active={taskView === "list"} onClick={() => setTaskView("list")} label="Task list" /><ViewTab active={taskView === "gantt"} onClick={() => setTaskView("gantt")} label="Gantt" /><ViewTab active={taskView === "calendar"} onClick={() => setTaskView("calendar")} label="Calendar" /><ViewTab active={taskView === "kanban"} onClick={() => setTaskView("kanban")} label="Kanban" /></div>{taskView === "list" && <div className="grid w-full gap-3 md:w-auto md:grid-cols-[minmax(220px,1fr)_180px_160px]"><label className="relative block"><span className="sr-only">Search tasks</span><span className="pointer-events-none absolute left-3 top-2.5 text-secondary" aria-hidden="true">⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search tasks" className={`${inputClass} pl-9`} /></label><label className="sr-only" htmlFor="task-status-filter">Filter by status</label><select id="task-status-filter" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as "All" | TaskStatus)} className={inputClass}><option value="All">All statuses</option>{taskStatuses.map((status) => <option key={status}>{status}</option>)}</select><label className="sr-only" htmlFor="task-priority-filter">Filter by priority</label><select id="task-priority-filter" value={priorityFilter} onChange={(event) => setPriorityFilter(event.target.value as "All" | TaskPriority)} className={inputClass}><option value="All">All priorities</option>{priorities.map((priority) => <option key={priority}>{priority}</option>)}</select></div>}</div>{taskView === "list" && <div className="divide-y divide-border">{filteredTasks.map((task) => <TaskRow key={task.id} task={task} statuses={taskStatuses} onStatusChange={(status) => changeTask(task.id, { status })} onEdit={() => openTaskEditor(task)} onDelete={() => void removeTask(task.id)} />)}{filteredTasks.length === 0 && <div className="rounded-lg border border-dashed border-border-strong px-4 py-10 text-center"><p className="text-sm font-medium text-primary">No tasks match these filters.</p><p className="mt-1 text-xs text-secondary">Try a different search or clear one of the filters.</p></div>}</div>}{taskView === "gantt" && <GanttChart tasks={ganttTasks} startDate={ganttStartDate} endDate={ganttEndDate} onTaskClick={(task) => { const source = project.taskList.find((item) => item.id === task.id); if (source) openTaskEditor(source); }} />}{taskView === "calendar" && <Calendar events={calendarEvents} onEventClick={(event) => { const source = project.taskList.find((item) => item.id === event.id); if (source) openTaskEditor(source); }} />}{taskView === "kanban" && <Kanban tasks={project.taskList} statuses={taskStatuses} onStatusChange={(taskId, status) => changeTask(taskId, { status })} onTaskClick={openTaskEditor} />}</CardContent></Card>
    <Modal open={editingTask !== null} title="Edit task" description="Update the task details and assignment." onClose={() => setEditingTask(null)}><form className="space-y-5" onSubmit={saveTask}><Input id="edit-task-title" label="Task name" value={taskForm.title} onChange={(event) => setTaskForm({ ...taskForm, title: event.target.value })} required /><Textarea id="edit-task-detail" label="Description" rows={3} value={taskForm.detail} onChange={(event) => setTaskForm({ ...taskForm, detail: event.target.value })} /><div className="grid gap-5 sm:grid-cols-2"><DateInput id="edit-task-start-date" label="Start date" value={taskForm.startDate} onChange={(event) => setTaskForm({ ...taskForm, startDate: event.target.value })} required /><DateInput id="edit-task-due-date" label="Due date" value={taskForm.dueDate} onChange={(event) => setTaskForm({ ...taskForm, dueDate: event.target.value })} /><Select id="edit-task-priority" label="Priority" value={taskForm.priority} onChange={(event) => setTaskForm({ ...taskForm, priority: event.target.value as TaskPriority })}>{priorities.map((priority) => <option key={priority}>{priority}</option>)}</Select></div><Select id="edit-task-assignee" label="Assignee" value={taskForm.assigneeId} onChange={(event) => setTaskForm({ ...taskForm, assigneeId: event.target.value })}><option value="">Unassigned</option>{workspaceMembers.map((member) => <option key={member.userId} value={member.userId}>{member.name}</option>)}</Select><div className="flex justify-end gap-3 border-t border-border pt-5"><Button type="button" variant="ghost" onClick={() => setEditingTask(null)}>Cancel</Button><Button type="submit">Save changes</Button></div></form></Modal>
...
function ProjectEditForm({ project, onSave, onCancel, onDelete, workspaceMembers }: { project: Project; onSave: (changes: { name: string; description: string; status: ProjectStatus; ownerId: string; startDate: string; dueDate: string; memberIds: string[] }) => Promise<void>; onCancel: () => void; onDelete: () => void; workspaceMembers: { userId: string; name: string }[] }) {
  ...
  return <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); if (!form.name.trim() || !form.memberIds.length) return; void onSave({ ...form, name: form.name.trim(), description: form.description.trim(), ownerId: form.memberIds.includes(form.ownerId) ? form.ownerId : form.memberIds[0] }); }}><Input id="edit-project-name" label="Project name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required /><Textarea id="edit-project-description" label="Description" rows={4} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} required /><MultiSelect id="edit-project-team" label="Project team" options={memberOptions} value={form.memberIds} onChange={(memberIds) => setForm({ ...form, memberIds, ownerId: memberIds.includes(form.ownerId) ? form.ownerId : memberIds[0] || "" })} placeholder="Choose team members" />{selectedTeam.length > 0 && <div className="flex items-center gap-3 rounded-lg border border-border bg-subtle px-3 py-2"><AvatarGroup people={selectedTeam} /><p className="text-xs text-secondary">{selectedTeam.length} team member{selectedTeam.length === 1 ? "" : "s"} selected</p></div>}<div className="grid gap-5 sm:grid-cols-2"><Select id="edit-project-status" label="Status" value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as ProjectStatus })}>{projectStatuses.map((status) => <option key={status}>{status}</option>)}</Select><Select id="edit-project-owner" label="Owner" value={form.ownerId} onChange={(event) => setForm({ ...form, ownerId: event.target.value })} disabled={!form.memberIds.length}><option value="">Select an owner</option>{workspaceMembers.filter((member) => form.memberIds.includes(member.userId)).map((member) => <option key={member.userId} value={member.userId}>{member.name}</option>)}</Select></div><div className="grid gap-5 sm:grid-cols-2"><DateInput id="edit-project-start-date" label="Start date" value={form.startDate} onChange={(event) => setForm({ ...form, startDate: event.target.value })} required /><DateInput id="edit-project-due-date" label="Target date" value={form.dueDate} onChange={(event) => setForm({ ...form, dueDate: event.target.value })} required /></div><div className="border-t border-border pt-5"><button type="button" onClick={onDelete} className="cursor-pointer text-sm font-medium text-danger underline underline-offset-4 hover:text-red-700">Delete project</button><div className="mt-5 flex justify-end gap-3"><Button type="button" variant="ghost" onClick={onCancel}>Cancel</Button><Button type="submit" disabled={!form.memberIds.length}>Save changes</Button></div></div></form>;
}
...
function formatTaskDate(rawDate: string | null) {
  ...
  return new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeZone: "UTC" }).format(new Date(`${year}-${month}-${day}T00:00:00Z`));
}
...
function formatInputDate(date: string) { if (!date) return noDueDate; const match = date.match(/^(\d{4})-(\d{2})-(\d{2})$/); if (!match) return noDueDate; const [, year, month, day] = match; return new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeZone: "UTC" }).format(new Date(`${year}-${month}-${day}T00:00:00Z`)); }