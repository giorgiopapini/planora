...
export function ProjectWorkspace({
  ...
  const [taskView, setTaskView] = useState<
    "list" | "gantt" | "calendar" | "kanban"
  >("list");
  const [isEditingProject, setIsEditingProject] = useState(false);
  const [isDeletingProject, setIsDeletingProject] = useState(false);
  const [projectConfirmation, setProjectConfirmation] = useState("");
  const [isDeletingProjectRequest, setIsDeletingProjectRequest] =
    useState(false);
  ...
  const taskStatuses = useMemo(
    () =>
      project.workflowStatuses.map((status) => status.name).length
        ? project.workflowStatuses.map((status) => status.name)
        : Array.from(
            new Map(
              project.taskList.map((task) => [task.status, task.status]),
            ).values(),
          ),
    [project.taskList, project.workflowStatuses],
  );
  const filteredTasks = useMemo(
    () =>
      project.taskList.filter((task) => {
        const matchesSearch =
          `${task.title} ${task.detail} ${task.tags.join(" ")}`
            .toLowerCase()
    ...
    () =>
      project.taskList
        ...
        .map((task) => ({
          id: task.id,
          name: task.title,
          start: task.startDateIso,
          end: task.dueDateIso as string,
          progress:
  ...
  const ganttStartDate =
    project.startDateIso > today ? project.startDateIso : today;
  ...
  const calendarEvents = useMemo(
    () =>
      project.taskList
        ...
        .map((task) => ({
          id: task.id,
          date: task.startDateIso,
          endDate: task.dueDateIso as string,
          title: task.title,
          color:
            task.status.toLowerCase() === "completed"
              ? ("dark" as const)
  ...
  async function saveProject(changes: {
    ...
    try {
      ...
      const owner = workspaceMembers.find(
        (member) => member.userId === changes.ownerId,
      );
      setProject((current) => ({
        ...current,
        name: changes.name,
        ...
        owner: owner?.name || current.owner,
        startDate: formatInputDate(changes.startDate),
        startDateIso: changes.startDate,
        dueDate: formatInputDate(changes.dueDate),
        dueDateIso: changes.dueDate,
        team: changes.memberIds.map((userId) => {
          ...
          return member
            ? {
                ...member,
                role:
                  userId === changes.ownerId ? "Project owner" : "Project team",
              }
            : { userId, name: "User", role: "Project team" };
        }),
      }));
  ...
  async function removeProject() {
    ...
    try {
      await deleteProject({
        ...
        projectName: projectConfirmation,
      });
      router.push(
        `/projects?workspace=${encodeURIComponent(project.workspaceId)}`,
      );
    } catch (actionError) {
      setError(
        actionError instanceof Error
  ...
  async function changeTask(taskId: string, changes: Partial<ProjectTask>) {
    ...
    try {
      await updateTaskAction({
        taskId,
        title: changes.title,
        description: changes.detail,
        startDate: changes.startDateIso,
        dueDate:
          changes.dueDate === undefined
            ? undefined
            : changes.dueDate === "No due date"
              ? null
              : toInputDate(changes.dueDate),
        priority: changes.priority,
        statusName: changes.status,
        assigneeIds: changes.assigneeIds,
      });
      setProject((current) => {
        const taskList = current.taskList.map((item) =>
          item.id === taskId
            ? {
                ...item,
                ...changes,
                ...(changes.dueDate !== undefined
                  ? {
                      dueDate:
                        changes.dueDate === noDueDate
                          ? noDueDate
                          : formatInputDate(toInputDate(changes.dueDate)),
                      dueDateIso:
                        changes.dueDate === noDueDate
                          ? null
                          : toInputDate(changes.dueDate),
  ...
  function openTaskEditor(task: ProjectTask) {
    ...
    setTaskForm({
      title: task.title,
      detail: task.detail === "No description provided." ? "" : task.detail,
      startDate: task.startDateIso,
  ...
  async function saveTask(event: FormEvent<HTMLFormElement>) {
    ...
    await changeTask(editingTask.id, {
      title: taskForm.title,
      detail: taskForm.detail,
      startDate: taskForm.startDate,
      dueDate: taskForm.dueDate || noDueDate,
      priority: taskForm.priority,
      assigneeIds: taskForm.assigneeId ? [taskForm.assigneeId] : [],
    });
    setEditingTask(null);
  }
  ...
  async function addTask(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const todoStatus = project.workflowStatuses.find(
        (status) => status.name.toLowerCase() === "todo",
      );
      const assignee = workspaceMembers.find(
        (member) => member.userId === taskForm.assigneeId,
      );
      setProject((current) => ({
        ...current,
        taskList: [
          {
            ...
            detail: taskForm.detail.trim() || "No description provided.",
            status: todoStatus?.name || "Todo",
            statusId: todoStatus?.id || "",
            startDate: formatInputDate(taskForm.startDate),
            startDateIso: taskForm.startDate,
            dueDate: taskForm.dueDate
              ? formatInputDate(taskForm.dueDate)
              : noDueDate,
            dueDateIso: taskForm.dueDate || null,
            assignees: assignee ? [assignee.name] : [],
            assigneeIds: assignee ? [assignee.userId] : [],
            priority: taskForm.priority,
            tags: [],
            tagIds: [],
          },
        ...
        tasks: { ...current.tasks, total: current.tasks.total + 1 },
      }));
...
function ViewTab({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${active ? "bg-surface text-primary ring-1 ring-border" : "text-secondary hover:text-primary"}`}
    >
      {label}
    </button>
  );
...
function ProjectEditForm({
  ...
  const [form, setForm] = useState({
    ...
    startDate: project.startDateIso,
    dueDate: project.dueDateIso,
    memberIds: project.team.map((member) => member.userId),
  });
  const selectedTeam = form.memberIds.map((userId) => ({
    name:
      workspaceMembers.find((member) => member.userId === userId)?.name ||
      "User",
  }));
    ...
    <form
      ...
      onSubmit={(event) => {
        ...
        void onSave({
          ...
          ownerId: form.memberIds.includes(form.ownerId)
            ? form.ownerId
            : form.memberIds[0],
        });
      }}
      ...
      <MultiSelect
        ...
        onChange={(memberIds) =>
          setForm({
            ...form,
            memberIds,
            ownerId: memberIds.includes(form.ownerId)
              ? form.ownerId
              : memberIds[0] || "",
          })
        }
...
function TaskRow({
  ...
  return (
    <article className="flex flex-col gap-4 rounded-lg px-3 py-4 transition-colors duration-120 hover:bg-subtle sm:flex-row sm:items-center">
      <div
        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border text-sm ${task.status.toLowerCase() === "completed" ? "border-accent bg-accent text-white" : "border-border-strong bg-surface text-transparent"}`}
        aria-hidden="true"
      >
        ✓
      </div>
      <div className="min-w-0 flex-1">
        <p
          className={`text-sm font-medium ${task.status.toLowerCase() === "completed" ? "text-secondary line-through" : "text-primary"}`}
        >
          {task.title}
        </p>
