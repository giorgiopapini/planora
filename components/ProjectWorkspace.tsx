...
export function ProjectWorkspace({ project: initialProject, selectedWorkspace, workspaceMembers = initialProject.team.map((member) => ({ userId: member.userId, name: member.name })) }: ProjectWorkspaceProps) {
  ...
  const calendarEvents = useMemo(() => project.taskList.filter((task) => task.dueDateIso).map((task) => ({ id: task.id, date: task.dueDateIso as string, title: task.title, color: task.status.toLowerCase() === "completed" ? "dark" as const : task.status.toLowerCase() === "todo" ? "muted" as const : "accent" as const })), [project.taskList]);
