...
import { createTask, deleteTask, updateProject, updateTask as updateTaskAction } from "@/app/actions";
...
export function ProjectWorkspace({ project: initialProject, selectedWorkspace, workspaceMembers = initialProject.team.map((member) => ({ userId: member.userId, name: member.name })) }: ProjectWorkspaceProps) {
  ...
  return <div className="space-y-8">
    ...
    <Modal open={isEditingProject} title="Edit project" description="Keep the project information clear and current for your team." onClose={() => setIsEditingProject(false)}><ProjectEditForm project={project} onSave={saveProject} onCancel={() => setIsEditingProject(false)} workspaceMembers={workspaceMembers} /></Modal>
...
function ProjectEditForm({ project, onSave, onCancel, workspaceMembers }: { project: Project; onSave: (changes: { name: string; description: string; status: ProjectStatus; ownerId: string; startDate: string; dueDate: string; memberIds: string[] }) => Promise<void>; onCancel: () => void; workspaceMembers: { userId: string; name: string }[] }) {
