...
export function ProjectWorkspace({ project: initialProject, selectedWorkspace, workspaceMembers = initialProject.team.map((member) => ({ userId: member.userId, name: member.name })) }: ProjectWorkspaceProps) {
  ...
  return <div className="space-y-8">
    ...
    <Modal open={isDeletingProject} title="Delete project?" description="This action cannot be undone." onClose={() => setIsDeletingProject(false)}><div className="space-y-5"><div className="rounded-lg border border-danger/30 bg-danger-soft p-4 text-sm leading-6 text-danger">Deleting <strong>{project.name}</strong> permanently removes this project, every task, team membership, milestone, and all other related data.</div><Input id="project-delete-confirmation" label={`Type “${project.name}” to confirm`} value={projectConfirmation} onChange={(event) => setProjectConfirmation(event.target.value)} autoComplete="off" /><div className="flex justify-end gap-3 border-t border-border pt-5"><Button type="button" variant="ghost" onClick={() => setIsDeletingProject(false)}>Cancel</Button><Button type="button" variant="danger" onClick={() => void removeProject()} disabled={projectConfirmation !== project.name}>Delete project</Button></div></div></Modal>
