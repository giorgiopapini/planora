...
export type Workspace = { id: string; name: string; slug: string; description: string | null; ownerId: string };
...
export async function requireWorkspace(searchParams: WorkspaceSearchParams): Promise<Workspace> {
  ...
  const { data: memberships, error } = await supabase.from("workspace_members").select("workspace_id, workspaces(id, name, slug, description, owner_id)").eq("user_id", user.id).eq("status", "active");
