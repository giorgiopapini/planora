...
type TaskRow = { id: string; project_id: string; status_id: string; title: string; description: string; priority: string; due_date: string | null; task_assignees: { user_id: string; profiles: ProfileRow | ProfileRow[] | null }[]; task_tags: { tag_id: string; tags: { id: string; name: string } | { id: string; name: string }[] | null }[] };
...
function displayDate(value: string | null | undefined) { if (!value) return "No due date"; return new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" }).format(new Date(`${value.slice(0, 10)}T00:00:00Z`)); }
...
async function getProjectDetails(project: Project) {
  ...
  const [{ data: tasks, error: tasksError }, { data: statuses, error: statusesError }, { data: members, error: membersError }, { data: milestones, error: milestonesError }, { data: activity, error: activityError }] = await Promise.all([
    supabase.from("tasks").select("id, project_id, status_id, title, description, priority, due_date, task_assignees(user_id, profiles!task_assignees_user_id_fkey(id, full_name, email)), task_tags(tag_id, tags(id, name))").eq("project_id", project.id).is("deleted_at", null).order("position").order("created_at"),
...
function mapProject(project: ProjectRow, count: { total: number; completed: number }): Project { const profile = one(project.profiles); const workspace = one(project.workspaces); const progress = count.total ? Math.round((count.completed / count.total) * 100) : 0; return { id: project.id, workspaceId: project.workspace_id, slug: project.slug, name: project.name, status: projectStatus(project.status), progress, detail: `${Math.max(0, count.total - count.completed)} tasks remaining`, description: project.description || project.short_description || "No description provided.", ownerId: project.owner_id, owner: profile?.full_name || profile?.email || "User", startDate: displayDate(project.start_date), dueDate: displayDate(project.due_date), workspace: workspace?.name || "Workspace", tasks: { completed: count.completed, total: count.total }, taskList: [], milestones: [], team: [], activity: [], workflowStatuses: [] }; }
function mapTask(task: TaskRow, status?: StatusRow): ProjectTask { const assignees = task.task_assignees.map((assignee) => { const profile = one(assignee.profiles); return { id: assignee.user_id, name: profile?.full_name || profile?.email || "User" }; }); const tags = task.task_tags.map((taskTag) => one(taskTag.tags)).filter((tag): tag is { id: string; name: string } => Boolean(tag)); return { id: task.id, title: task.title, status: status?.name || titleCase(status?.key || "todo"), statusId: task.status_id, dueDate: displayDate(task.due_date), assignees: assignees.map((person) => person.name), assigneeIds: assignees.map((person) => person.id), priority: taskPriority(task.priority), tags: tags.map((tag) => tag.name), tagIds: tags.map((tag) => tag.id), detail: task.description || "No description provided." }; }
