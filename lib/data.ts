...
function mapTask(task: TaskRow, status?: StatusRow): ProjectTask { const assignees = task.task_assignees.map((assignee) => { const profile = one(assignee.profiles); return { id: assignee.user_id, name: profile?.full_name || profile?.email || "User" }; }); const tags = task.task_tags.map((taskTag) => one(taskTag.tags)).filter((tag): tag is { id: string; name: string } => Boolean(tag)); return { id: task.id, title: task.title, status: status?.name || titleCase(status?.key || "todo"), statusId: task.status_id, dueDate: displayDate(task.due_date), dueDateIso: task.due_date ? task.due_date.slice(0, 10) : null, assignees: assignees.map((person) => person.name), assigneeIds: assignees.map((person) => person.id), priority: taskPriority(task.priority), tags: tags.map((tag) => tag.name), tagIds: tags.map((tag) => tag.id), detail: task.description || "No description provided." }; }
